...
test.group('PasswordResetToken token | decode', () => {
  test('decode "{input}" as token')
    ...
    .run(({ assert }, { input, output }) => {
      const decoded = PasswordResetToken.decode(input as string)
      if (!decoded) {
...
test.group('PasswordResetToken token | create', () => {
  test('create a transient token', ({ assert }) => {
    ...
    const token = PasswordResetToken.createTransientToken(1, 40, '20 mins')
  ...
  test('create token with a secret', ({ assert }) => {
    ...
    const transientToken = PasswordResetToken.createTransientToken(1, 40, '20 mins')
    ...
    const decoded = PasswordResetToken.decode(token.value!.release())
